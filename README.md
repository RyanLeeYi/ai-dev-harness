...
- **評估設計** —— 凍結驗收標準、生成者 ≠ 驗收者、獨立性建立在 fresh context 而非換模型
...
| | [L5 檢查者的獨立性](#c--檢查生成者--驗收者) | fresh context + 唯讀凍結輸入(跨模型層 2026/08 移除) |
...
- **執行者**(範圍已定的實作)—— 只有一支 `executor`(機械改動與有界技術判斷都歸它),frontmatter 釘 sonnet。目的不是獨立性,是**省額度**:工作跑在獨立 context,主 session 只收最後一則訊息。停用委派、不收安全敏感工作 —— 它的契約是「照規格做完、不多想」,而安全最需要的正是「規格本身有沒有漏洞」那個判斷。**預設是主 session 直接做**(direct-first):要不要派、派幾個,2026/08/19 起一律過 [baton](https://github.com/cablate/baton) 的 dispatch brake 五問(Outcome／Direct-work／Independence／Ownership／Closure,任一答不出就不派),本地不再維護第二份委派規則;只保留 baton 沒有的不可外包清單與五問對應的本地機制(acceptance 凍結、`harness-plan` 執行波次、`touches` 擁有權閘門)—— 實測完整編排的 token 與延遲成本大多落在主 session,不是落在便宜的 child
- **`executor` 一律 worktree** —— 由 `PreToolUse` hook 硬擋,沒設就 deny,不看工作樹髒不髒:`git status` 分不出髒是誰造成的,也預測不了別的視窗稍後才開始寫。同一個思路 —— **能寫進 hook 的,就不要只寫進文件**。代價差一個數量級:多開一個 worktree 是 2 秒 + 260MB,該開沒開是 1–2 萬 token 的污染診斷加靜默覆寫風險。唯讀角色不擋,它們污染不了別人。**但 worktree 是從 `origin/<預設分支>` 開的,不是從你的 HEAD** —— commit 了沒 push 的規格 worker 一樣看不到,而 `git status` 是乾淨的。所以規格原文寫進派工 prompt,並明講「prompt 這份就是權威,不要回去查 feature list,查不到也不要停」;worker 說「我看到的跟你講的不一樣」時先驗它的基準,不要先假設它讀錯(2026-08-19 連兩輪派工報廢買來的,兩次 worker 都是對的)
...
> ⚠️ 切模型時要注意:別讓代理接管模式破壞檢查者的 context 隔離(見下方教訓)。
